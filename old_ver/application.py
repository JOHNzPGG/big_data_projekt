...
json_schema = StructType([
    ...
    StructField("is_minor", BooleanType(), True), # <-- DODANO
    StructField("lat", DoubleType(), True), # <-- DODANO
    StructField("lon", DoubleType(), True)  # <-- DODANO
])
...
def start_app():
    ...
    # 3. Logika "Wojen Edycyjnych" z detekcją Revertów
    edit_wars_stream = (
        parsed_stream
        .withWatermark("timestamp", "5 minutes")
        # Nowa kolumna tymczasowa: czy to jest revert?
        # Szukamy po angielsku, polsku, hiszpańsku itp.
        .withColumn("is_revert",
                    when(
                        lower(col("comment")).rlike("revert|undid|undo|wycofano|przywrócono|revertido"), 1
                    ).otherwise(0)
                    )
        .groupBy(
        ...
        .agg(
            ...
            sum("is_revert").alias("reverts_count")  # <-- NOWA STATYSTYKA: Liczba cofnięć w oknie
        )
        # Filtr: Łapiemy, jeśli są 2+ edycje ATAKŻE jeśli był chociaż jeden revert (To zaostrza definicję wojny)
    ...
    # --- STRUMIEŃ MAPY NA ŻYWO ---
    def update_live_map(df, epoch_id):
        try:
            df.persist()
            if df.count() > 0:
                # Nadpisujemy zawsze ten sam plik: "live_map.csv"
                df.select("lat", "lon").coalesce(1).write.mode("overwrite").option("header", "true").csv(
                    TEMP_DIR + "_map")
                files = glob.glob(f"{TEMP_DIR}_map/*.csv")
                if files:
                    shutil.move(files[0], os.path.join(OUTPUT_MAP, "live_map.csv"))
            df.unpersist()
        except:
            pass

    # Filtrujemy tylko zdarzenia, które mają współrzędne (czyli z adresów IP)
    map_stream = parsed_stream.filter(col("lat").isNotNull() & col("lon").isNotNull())

    query_map = (
        map_stream.writeStream
        .outputMode("append")
        .foreachBatch(update_live_map)
        .trigger(processingTime="5 seconds")  # Aktualizacja co 5 sekund!
        .option("checkpointLocation", CHECKPOINT_DIR + "_map")
        .start()
    )
