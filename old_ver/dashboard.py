...
if df.empty:
    ...
    st.subheader("🌍 Radar Niezalogowanych (Aktywność IP na żywo)")
    try:
        map_path = os.path.join("output_live_map", "live_map.csv")
        if os.path.exists(map_path):
            live_map_df = pd.read_csv(map_path)

            # Wymuszamy poprawne nazwy kolumn dla st.map
            if 'lat' in live_map_df.columns and 'lon' in live_map_df.columns:
                # Odrzucamy puste wiersze
                live_map_df = live_map_df.dropna(subset=['lat', 'lon'])

                if not live_map_df.empty:
                    st.map(live_map_df, color="#ff4b4b", size=50)
                else:
                    st.info("🛰️ Mapa gotowa, ale w ciągu ostatnich 5 sekund nie było edycji z niezalogowanych IP.")
            else:
                st.warning("W pliku live_map.csv brakuje kolumn 'lat' i 'lon'.")
        else:
            st.info("⏳ Oczekiwanie na pierwszy plik mapy od Sparka (live_map.csv)...")
    ...
    with col_left:
        ...
        display_df = dedup_df[
            ['title', 'total_edits', 'reverts_count', 'unique_users', 'combatants', 'latest_comment', 'start_time',
        ...
        display_df.rename(columns={
            ...
            'reverts_count': 'Liczba Cofnięć',  # <-- NOWA KOLUMNA
            'unique_users': 'Konta',
            'combatants': 'Walczący',
